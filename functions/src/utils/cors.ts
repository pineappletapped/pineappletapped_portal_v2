...
export interface CorsOptions {
  /**
   * Additional origin patterns to allow for a specific request.
   * Patterns follow the syntax `scheme://host[:port]` with optional wildcards:
   *   - `*` allows any host when used as the hostname.
   *   - `*.example.com` allows the apex domain and any subdomain.
   *   - `:*` allows any port for the specified host.
   */
...
const defaultOrigins = new Set(
  DEFAULT_ORIGIN_PATTERNS.map((pattern) => pattern.trim()).filter((pattern) => pattern.length > 0),
);
...
function compileOriginPattern(pattern: string): OriginMatcher | null {
...
function compileOriginPatterns(patterns: string[]): OriginMatcher[] {
  const compiled: OriginMatcher[] = [];
...
function appendVaryHeader(res: ExpressResponse, value: string) {
...
function buildHeaderValue(values: string[]): string {
  return Array.from(new Set(values.map((value) => value.trim()).filter((value) => value.length > 0))).join(',');
}

export const CALLABLE_CORS_ORIGINS: true | (string | RegExp)[] = true;

export function runCorsCheck(
  req: ExpressRequest,
  res: ExpressResponse,
  options: CorsOptions = {},
): CorsResult {
